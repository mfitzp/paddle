...
from .globals import settings, STATUS_QCOLORS, custom_pyqtconfig_hooks
...
class SVGMplView(QSvgWidget):
    """
    An SVG-based matplotlib view that does not use the Agg backend. Using
    bbox_inches to auto-pad and scale image to the available window size.
    ...
    def redraw(self):
        ...
        # Adjust fig size to window shape
        s = self.size()
        dpi = self.figure.get_dpi()
        w, h = s.width() / dpi, s.height() / dpi
        self.figure.set_size_inches(w,h)

        # Write image data
        imgdata = io.BytesIO()
        self.figure.savefig(imgdata, format='svg', pad_inches=0.5, bbox_inches='tight')
        imgdata.seek(0)
        self.load(QByteArray(imgdata.read()))
...
class ConfigPanel(QWidget):
    ...
    def addBottomSpacer(self, gd):
        gd.addItem(QSpacerItem(1, 1, QSizePolicy.Minimum, QSizePolicy.Minimum), gd.rowCount(), 0)
...
class DialogAbout(QDialog):
    def __init__(self, parent, **kwargs):
        ...
        self.setWindowTitle('About Paddle')
...
class DialogRegister(QDialog):
    def __init__(self, parent, **kwargs):
        ...
        self.setWindowTitle('Register Paddle')

        self.layout = QVBoxLayout()
        self.layout.addWidget(QLabel('Please register Paddle by entering your details below.\n\nThis is completely optional but helps it helps us find out\nhow Paddle is being used.'))
...
class ToolListWidget(QListWidget):
    ...
    def showContext(self, pos):
        ...
        if not item.tool.is_disableable:
            return
